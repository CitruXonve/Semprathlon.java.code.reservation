	...
	public static void main(String[] args) throws IOException{
		...
		while(in.nextLine()!=null){
			...
			for(int i=Math.min(k, n);i<n;i++){
				for(int j1=v1;j1>=ob[i].w1;j1--)
					for(int j2=v2;j2>=0;j2--){
							f[i&1][j1][j2]=Math.max(f[i&1][j1][j2], f[(i&1)^1][j1-ob[i].w1][j2]+ob[i].v);
							ans=Math.max(ans, f[i&1][j1][j2]);
					}
				for(int j2=v2;j2>=ob[i].w2;j2--)
					for(int j1=v1;j1>=0;j1--){
							f[i&1][j1][j2]=Math.max(f[i&1][j1][j2], f[(i&1)^1][j1][j2-ob[i].w2]+ob[i].v);
							ans=Math.max(ans, f[i&1][j1][j2]);
					}
			}
